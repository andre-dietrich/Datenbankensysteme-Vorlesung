...
**Aufgabe:** Schreiben Sie eine Abfrage, die folgende Spalten aus der Tabelle `Products` anzeigt und umbenennt:
- `product_id` → `id`
- `name` → `product_name`
- `price` → `price_eur`
- `category` (ohne Umbenennung)
...
**Aufgabe:** Schreiben Sie eine Abfrage, die für jede Kategorie folgende Statistiken berechnet:
- `category`
- `total_products` (Anzahl mit COUNT)
- `avg_price` (Durchschnittspreis mit AVG)
- `cheapest` (günstigstes Produkt mit MIN)
- `most_expensive` (teuerstes Produkt mit MAX)
- `total_stock` (Gesamtbestand mit SUM)
